...
// Format region codes (e.g., euw1 -> EUW)
function formatRegionLabel(region) {
  if (!region) return '[Unknown]';
  return region.replace(/[0-9]/g, '').toUpperCase();
}

// Async PDF-Generator!
// Date string helper for footer
function formatDataDate(dateValue, now) {
...
export const generatePlayerPDF = async (playerData, championMastery, leagueDetails, riotId, summonerInfo, normalStats, region = 'euw1', dataDate = 'today') => {
  ...
  doc.text(`Region: ${formatRegionLabel(region)}`, infoX, profileY + 18, { align: 'right' });
  ...
  doc.setFontSize(10);
  doc.text(`Date: ${formatDataDate(dataDate, now)}`, 10, 290);
